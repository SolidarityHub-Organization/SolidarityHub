...
namespace LogicPersistence.Api.Controllers 
{
    ...
    public class AffectedZoneController : ControllerBase {
        ...
        [HttpGet("affected-zones/paginated")]
		public async Task<IActionResult> GetPaginatedAffectedZonesAsync([FromQuery] int page = 1, [FromQuery] int size = 10)
		{
			try
			{
				var (affectedZones, totalCount) = await _affectedZoneServices.GetPaginatedAffectedZonesAsync(page, size);
